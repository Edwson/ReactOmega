...
const NebulaShader = {
  ...
  `,
    ...
    void main() {
      ...
      // Add density control
      noise = noise * uDensity;
      
      // Soften edges
      float alpha = smoothstep(0.0, 1.0, noise + 0.5);
...
function NebulaMesh({ speed, nebulaColor, density }: { speed: number; nebulaColor: string; density: number }) {
    const meshRef = useRef<any>(null);

    const uniforms = useMemo(
        () => ({
            uTime: { value: 0 },
            uColor: { value: new Color(nebulaColor) },
    ...
    useFrame((state) => {
        if (meshRef.current) {
            ...
            meshRef.current.material.uniforms.uColor.value.set(nebulaColor);
...
/**
 * Nebula - Colorful gas cloud effect with flowing particles
 */
    ...
    return (
        ...
        >
            <Canvas camera={{ position: [0, 0, 1] }}>
                <NebulaMesh speed={speed} nebulaColor={nebulaColor} density={density} />
            </Canvas>
        </div>
    );
});
